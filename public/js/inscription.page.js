...
function showError(pMessage) {
	$('#errors').empty();
	$('.saisi').css('background-color', '#ff7d7d')
	$('input').css('background-color', '#ff7d7d')
	$('#errors').append(`<span class="errorSaisi">${pMessage}</span>`)
}
...
$('#submit').on('click', ()=>{
	...
	if (!newUser.email.match(regexEmail)) {
		...
		showError("le pseudo a moins 8 caractere ")
		errors = true;
	} if ($('#password').val().length < 8) {
		showError("le mot de passe a moins 8 caractere ")
		errors = true;
	}
