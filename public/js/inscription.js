...
$('#submit').on('click', ()=>{
	...
	newUser["username"] = $('#pseudo').val()
	newUser["password"] = txtToSHA256($('#password').val());
	
	if (newUser.email.match(regexEmail) && newUser.username !="" && newUser.password !="e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855") {
		$('#errors').empty();
		$('#errors').append(`<span style="color : green" class="errorSaisi">Vous etes inscris! </span>`)
		socket.emit("userSignUp", newUser);
	} else {
		$('#errors').empty();
		if (!newUser.email.match(regexEmail)) {
			$('#errors').append(`<span class="errorSaisi">Email invalide ! </span>`)
		}
		if (newUser.username =="") {
			$('#errors').append(`<span class="errorSaisi">pseudo invalide ! </span>`)
		}
		if (newUser.password =="e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855") {
			$('#errors').append(`<span class="errorSaisi">mot de passe invalide ! </span>`)
		}
	}
